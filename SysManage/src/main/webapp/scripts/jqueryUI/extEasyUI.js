...
/**
 * @author 孙宇
 * @requires jQuery,EasyUI 通用错误提示 用于datagrid/treegrid/tree/combogrid/combobox/form加载数据出错时的操作
 */
var easyuiErrorFunction = function(XMLHttpRequest) {
	if (parent.$ && parent.$.messager) {
		parent.$.messager.progress('close');
		parent.$.errorDialog(XMLHttpRequest.responseText);// 提示服务器异常
		// parent.$.messager.alert('错误', XMLHttpRequest.responseText, "warning");
	} else {
		$.messager.progress('close');
		$.errorDialog(XMLHttpRequest.responseText);// 提示服务器异常
	}
};
...
/**
 * Datagrid扩展方法tooltip 基于Easyui 1.3.3，可用于Easyui1.3.3+ 简单实现，如需高级功能，可以自由修改 使用说明: 在easyui.min.js之后导入本js 代码案例:
 * $("#dg").datagrid('tooltip'); 所有列 $("#dg").datagrid('tooltip',['productid','listprice']); 指定列
 * 
 * @author 夏悸
 */
...
/**
 * Treegrid扩展方法tooltip 基于Easyui 1.3.3，可用于Easyui1.3.3+ 简单实现，如需高级功能，可以自由修改 使用说明: 在easyui.min.js之后导入本js 代码案例:
 * $("#dg").treegrid('tooltip'); 所有列 $("#dg").treegrid('tooltip',['productid','listprice']); 指定列
 * 
 * @author 夏悸
 */
...
/**
 * @author 夏悸
 * @requires jQuery,EasyUI 扩展tree，使其可以获取实心节点
 */
 ...
 * @returns $.modalDialog.xxx 这个xxx是可以自己定义名称，主要用在弹窗关闭时，刷新某些对象的操作，可以将xxx这个对象预定义好
 */
$.modalDialogThree = function(options) {
	if ($.modalDialogThree.handler == undefined) {// 避免重复弹出
		var opts = $.extend({
			title : '',
			width : 840,
			height : 680,
			modal : true,
			onClose : function() {
				$.modalDialogThree.handler = undefined;
				$(this).dialog('destroy');
			},
			onOpen : function() {
				// parent.$.messager.progress({
		...
		opts.modal = true;// 强制此dialog为模式化，无视传递过来的modal参数
		return $.modalDialogThree.handler = $('<div/>').dialog(opts);
	}
};
/**
 * 自定义服务器异常提示框
 ...
 * @author 梅江顺
 */
$.errorDialog = function(errorhtml, options) {
	if ($.errorDialog.handler == undefined) {// 避免重复弹出
		var opts = $.extend({
			title : '服务器异常',
			width : 400,
			height : 230,
			modal : true,
			onClose : function() {
				$.errorDialog.handler = undefined;
				$(this).dialog('destroy');
			},
			buttons : [ {
				text : '关闭',
				handler : function() {
					$.errorDialog.handler.dialog('close');
				}
			} ]
		}, options);
		opts.modal = true;// 强制此dialog为模式化，无视传递过来的modal参数
		opts.content = errorhtml;
		return $.errorDialog.handler = $('<div/>').dialog(opts);
	}
};

/**
 * @author 孙宇
 * @requires jQuery,EasyUI 扩展datagrid，添加动态增加或删除Editor的方法 例子如下:
 * 
 * <pre>
 * datagrid.datagrid('removeEditor', 'cpwd');//第二个参数可以是数组
