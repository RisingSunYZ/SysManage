	...
	var methods = {
		init : function(options) {
			var settings = $.extend({
				...
				dataType : 'json',
				// 上传状态(prepare/start/uploading/error/complete)
				status : 'prepare',
				// 是否将文件信息存入数据库
				savefile : true,
				// 选择文件后自动提交
		...
		submit : function() {
			...
			if (validefiletype($this, getExt(filename))) {
				...
				var progressfn = function() {
					$.ajax({
						...
						success : function(data) {
							...
							// 上传已经完成或者出错，强制停止进度条
							if (settings.status == "error" || settings.status == "complete") {
								loaded = total;
								percent = 100;
							}
							// 显示进度条
							settings.progress(settings.status, filename, loaded, total, percent);
							// 结束进度条
							if (percent == 100) {
				...
				$.ajaxFileUpload.call($this, {
					...
					data : {
						save : settings.savefile
					},
					before : function() {
						// 开始上传
						settings.status = "uploading";
					},
					success : function(data, status) {
						...
						if (data.success) {
							...
							settings.error(true, null, data.msg);
						}
					},
